...
def operas_both_at_premiere(guest, composer):
    ...
    """
    artist_set = set([opera.author for opera in operas])
    if not (guest in artist_set and composer in artist_set):
        raise ValueError
    author_dict = {
        _get_date(line.date): line.play for line in operas if line.author == composer
    }
    born_guest = _get_date(composers[guest].born)
    die_guest = _get_date(composers[guest].died)
    born_composer = _get_date(composers[composer].born)
    die_composer = _get_date(composers[composer].died)

    res_list = [
        play
        for _date, play in author_dict.items()
        if born_composer < _date < die_composer and born_guest < _date < die_guest
    ]
...
def operas_both_at_premiere(guest, composer):
    if guest not in composers or composer not in composers:
        raise ValueError('Guest and/or composer are not valid composers')
