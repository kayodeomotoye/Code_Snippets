...
def extract_quotes(html: str = HTML) -> dict:
    ...
    #HTML_FMT= (html2text.html2text(HTML)).split('\\')
    TAG_RE = re.compile(r'<[^>]+>')
    ald =TAG_RE.sub('', HTML)
    HTML_FMT = re.sub('[0-9"]', '', ald)
    new_dict = {}
    for line in HTML_FMT.splitlines()[9:19]:
        #splitter = re.sub('[\n0-9"]', ' ', line).strip('. ').split('  ')
        splitter = line.strip('   .').split('-') 
        new_dict[splitter[1].lstrip()] = splitter[0].strip()
