...
def make_character_index(text=text, characters=CHARACTERS):
    ...
    split_text = enumerate(text.lower().splitlines())
    new_dict ={}
    for line in characters:
        if isinstance(line, tuple): new_dict[line[0]] = line
        else: new_dict[line] = line
    
    for text_line in split_text: 
        for key, value in new_dict.items():
            if isinstance(value, tuple):
                tup_line = value
                for line in tup_line:
                    if line.lower() in text_line[1]:
                        ret_dict[key.lower()].append(text_line[0])

            elif value.lower() in text_line[1]:
                ret_dict[key.lower()].append(text_line[0])

    final_dict={}
    for k, v in ret_dict.items():
        final_dict[k] = sorted(list(set(v)))            
